...
| icon        | INT          | 用户头像ID，范围1-9，对应public/icon目录下的头像图片 |
| is\_active  | BOOLEAN      | 是否有效（用于逻辑删除，默认为有效） |
| created\_at | TIMESTAMP    | 创建时间               |
| updated\_at | TIMESTAMP    | 更新时间               |
...
**描述：** 该表记录用户的行为，如收藏的招聘信息和投递的招聘信息。每个用户对每个职位只记录一次收藏和一次投递。

| 字段名                | 类型                    | 描述                                 |
| ------------------ | --------------------- | ---------------------------------- |
| id                 | SERIAL                | 唯一标识                               |
| user\_id           | INT                   | 用户ID，关联 `users` 表的 id              |
| favorite_job_ids   | ARRAY                 | 数组类型，存储用户收藏的招聘信息ID列表            |
| application_job_ids| ARRAY                 | 数组类型，存储用户投递的招聘信息ID列表            |
| updated_at         | TIMESTAMP             | 更新时间                               |
| created\_at        | TIMESTAMP             | 创建时间                               |
### 2.5 用户行为表 (user_actions) 更新

**新增字段：**
| 字段名     | 类型   | 描述                                          |
| --------- | ------ | --------------------------------------------- |
| job_state | JSONB[] | 二元数组，记录[招聘信息ID, 投递状态ID]的映射关系 |
...
**新增函数：**
- `update_job_application_state(user_id, job_id, state_id)`: 更新指定用户、指定职位的投递状态
- `get_job_application_state(user_id, job_id)`: 获取指定用户、指定职位的投递状态

**更新函数：**
- `add_application_job(user_id, job_id)`: 修改为投递时同时设置默认状态(1-已投递)
...
**描述：** 该表记录招聘信息的详细内容，包括职位名称、公司、发布时间等。views_count、favorites_count和applications_count字段分别记录浏览、收藏和投递的次数，每个用户对每个职位只记录一次。
...
| category_id                 | ARRAY        | 数组类型，存储该招聘信息所属的分类ID列表 |
...
| views\_count                | INT          | 浏览人数（每个用户只记录一次）              |
| favorites\_count            | INT          | 收藏人数（每个用户只记录一次）              |
| applications\_count         | INT          | 投递人数（每个用户只记录一次）              |
...
17	地产/建筑	17	0	TRUE
18	其他	18	0	TRUE
19  24h新开 19  0   TRUE  # 特殊分类，由触发器自动维护，包含24小时内更新的职位
20  往届可投 20 0   TRUE  # 特殊分类，由触发器自动维护，包含允许往届生投递的职位
...
| time_tag_id  | INT | 时间相关标签ID，关联 `tags` 表          |
| action_tag_id| INT | 行为相关标签ID，关联 `tags` 表，可为NULL   |
...
> **注意：** 以下是根据3.6临时查看数据库结构部分的实际数据库情况更新的函数与触发器说明。

### 3.0 数据库递归调用问题修复记录

在数据库中发现了一个严重的递归调用问题，导致堆栈溢出错误：
...
**问题原因：**
`check_membership_before_operation()` 和 `check_membership_expiration()` 函数之间存在相互递归调用，导致无限循环：
1. `check_membership_before_operation()` 调用 `check_membership_expiration()`
2. `check_membership_expiration()` 更新 `user_info` 表
3. 更新 `user_info` 表触发 `check_membership_before_operation()`
4. 循环重复直到堆栈溢出

**解决方案：**
1. 创建 `membership_check_log` 表记录检查时间
2. 修改 `check_membership_expiration()` 和 `check_membership_before_operation()` 函数，添加时间间隔检查
3. 避免函数之间的相互调用，让每个函数独立执行更新操作
4. 创建安全的更新函数 `safe_update_user()` 和 `safe_update_user_info()` 作为备选方案

**修复后的函数：**
```sql
-- 创建会员检查日志表
CREATE TABLE IF NOT EXISTS membership_check_log (
  id SERIAL PRIMARY KEY,
  last_check TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 修改check_membership_expiration函数，添加防递归保护
CREATE OR REPLACE FUNCTION check_membership_expiration()
RETURNS TRIGGER AS $$
DECLARE
  last_check_time TIMESTAMP;
  check_interval INTERVAL = '10 minutes';
BEGIN
  -- 获取上次检查时间
  SELECT MAX(mcl.last_check) INTO last_check_time
  FROM membership_check_log mcl;

  -- 如果上次检查是在指定间隔内，则跳过
  IF last_check_time IS NOT NULL AND (CURRENT_TIMESTAMP - last_check_time) < check_interval THEN
    RETURN NULL;
  END IF;

  -- 记录本次检查时间
  INSERT INTO membership_check_log (last_check) VALUES (CURRENT_TIMESTAMP);
  ...
  SET membership_remaining_days =
      CASE
          WHEN membership_end_date IS NOT NULL THEN
              GREATEST(0, EXTRACT(DAY FROM (membership_end_date - CURRENT_TIMESTAMP)))
          ELSE 0
      END,
      membership_type =
      CASE
          WHEN membership_end_date < CURRENT_TIMESTAMP THEN 'common_user'
...
-- 修改check_membership_before_operation函数，防止递归
CREATE OR REPLACE FUNCTION check_membership_before_operation()
RETURNS TRIGGER AS $$
DECLARE
  last_check_time TIMESTAMP;
  check_interval INTERVAL = '10 minutes';
BEGIN
  ...
  -- 直接执行更新，不再调用check_membership_expiration
  UPDATE user_info
  SET membership_remaining_days =
      CASE
          WHEN membership_end_date IS NOT NULL THEN
...
该函数用于自动更新招聘信息的is_24hnew和is_pregraduation标记，这两个标记分别用于标识当天发布的职位和允许往届生投递的职位。is_24hnew标记是根据post_time字段判断，只有发布日期是当天的职位才会被标记为true。

```sql
CREATE OR REPLACE FUNCTION update_job_classification_on_change()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    -- 更新24h新开标记（使用post_time而非last_update）
    NEW.is_24hnew := (DATE(NEW.post_time) = CURRENT_DATE);
...
该函数用于更新所有分类的计数，包括普通分类和特殊分类（24h新开、往届可投）。

```sql
CREATE OR REPLACE FUNCTION update_category_counts_core()
RETURNS VOID AS $$
DECLARE
    cat_id INT;
    current_grad_year INT;
...
该部分描述了用户行为（浏览、收藏、投递）的记录逻辑：

1. **浏览计数**：
   - 用户点击卡片的任意区域都会触发浏览计数
   - 使用localStorage存储已浏览的职位ID，确保每个用户对每个职位只记录一次浏览
   - 浏览计数会更新job_recruitments表中的views_count字段
   - 浏览计数会触发job_tags表中的action_tag_id字段更新，按规则选出浏览人数多的招聘信息

2. **收藏功能**：
   - 用户必须登录才能使用收藏功能
   - 每个用户对每个职位只记录一次收藏
   - 收藏会更新job_recruitments表中的favorites_count字段
   - 收藏会更新user_actions表中的favorite_job_ids数组

3. **申请投递功能**：
   - 用户必须登录才能使用申请投递功能
   - 每个用户对每个职位只记录一次投递
   - 投递会更新job_recruitments表中的applications_count字段
   - 投递会更新user_actions表中的application_job_ids数组
   - 如果有外部申请链接，只有在用户登录并记录投递行为后才会打开链接
...
BEGIN
    ...
    SET active_job_count = (
        ...
        FROM job_recruitments
        WHERE is_active = TRUE
        AND DATE(post_time) = CURRENT_DATE
    )
    WHERE id = 19;
...
BEGIN
    ...
    -- 更新24h新开标记（使用post_time而非last_update）
    UPDATE job_recruitments
    SET is_24hnew = (DATE(post_time) = CURRENT_DATE)
    WHERE is_active = TRUE;
